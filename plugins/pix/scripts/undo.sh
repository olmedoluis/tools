...
function undo() {
    ...
    local files=($(git diff --name-only) $(git ls-files --others --exclude-standard))
    ...
    if [[ ${#files} != 0 ]]; then
        ...
        echo -e "${RED}${IERR}No files to undo.${END_COLOR}"
        return 1
    fi
    
    if [[ ${#files[@]} == "${#matches[@]}" || ${#matches[@]} == 1 ]]; then
        echo -e "${IINFO}Undoing files."
        for file in "${matches[@]}"; do
            if some "$file" "${untracked_files[@]}"; then
                rm "$file"
            else
                git checkout -- "$file"
            fi
    ...
    while [ true ]; do
        ...
        for file in "${matches[@]}"; do
            if some "$file" "${undone_files[@]}"; then
        ...
        case $option in
            a)
                undone_files=("${matches[@]}")
                echo -e "${TAB}${GREEN}${ISTAR}${IOK}All files will be undone.${END_COLOR}"
                break
            ;;
            s)
                echo -e "${TAB}${GREEN}${ISTAR}${IOK}Selected files will be undone.${END_COLOR}"
                break
            ;;
            q)
                echo -e "${TAB}${YELLOW}${ISTAR}${IWARN}No files will be undone.${END_COLOR}"
                return 0
            ;;
            h)
                echo -e "${TAB}${ISTAR}${IINFO}Available options:"
                echo -e "${DTAB}${ISTAR}([number]): File option index."
                echo -e "${DTAB}${ISTAR}(a): Undo all files."
            ...
            *)
                ...
                if some "$option" "${undone_files[@]}"; then
                    local new_staged_files=($(filter "$option" "${undone_files[@]}"))
                    
                    undone_files=("${new_staged_files[@]}")
                    echo -e "${TAB}${YELLOW}${ISTAR}${IOK}File will be ignored.${END_COLOR}"
                else
                    undone_files+=("$option")
                    echo -e "${TAB}${GREEN}${ISTAR}${IOK}File will be undone.${END_COLOR}"
                fi
            ;;
        esac
    done
